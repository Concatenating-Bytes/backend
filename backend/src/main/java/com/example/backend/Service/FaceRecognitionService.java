...
public class FaceRecognitionService {
    ...
    public FaceVerifyResult verifyFace(UUID userId, String image, String ip) {
        ...
        try {
            response = pythonFaceWebClient.post()
                    .uri("/verify")
                    .bodyValue(req)
                    .retrieve()
        ...
        Float similarity = response.get("similarity") != null
        ...
        result.setMessage(verified ? "Verified" : "Failed");
    ...
    public FaceVerifyResult identify(String image) {
        ...
        FaceVerifyResult result = new FaceVerifyResult();
        result.setSimilarity(response.get("similarity") != null
                ? ((Number) response.get("similarity")).floatValue()
                : 0.0f);
        ...
        if ("unknown".equalsIgnoreCase(userId)) {
            ...
            result.setMatched_user_id(UUID.fromString(userId));
            result.setMessage("User identified");
        }
